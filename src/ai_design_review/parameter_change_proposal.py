...
APPLICABLE_ACTION_TYPES = {"propose_parameter_patch", "propose_tolerance_patch", "proposal_constraint"}
...
def apply_parameter_change_proposal(
    ...
    *,
    ...
    applied["summary"] = f"已应用参数修改方案，共更新 {len(refreshed.get('direct_changes') or []) + len(refreshed.get('synchronized_changes') or [])} 项参数。"
    ...
    applied_review.setdefault("agent_actions", []).append(
        {
            ...
            "turn_created_at": stored.get("source_turn_created_at"),
            "restandardized": True,
            "restandardization_status": "completed",
        }
    )
    return applied_review, {"proposal": _public_proposal(applied), "log_id": log_id, "patches": patches}
...
def _resolve_proposal(
    ...
    *,
    ...
    if constraints and not actionable and not questions:
        questions.append("当前约束已经满足；如需继续修改，请再提供一个明确目标参数。")
    if not actionable and not constraints and not questions:
        questions.append("请提供需要修改的参数及明确目标值。")
    ...
    count = len(direct_changes) + len(synchronized_changes)
    covered_fields = {
    ...
    summary = {
        "ready": f"方案已完成整体校验，将同步更新 {count} 项参数。",
        "warning": f"方案将同步更新 {count} 项参数，但仍有风险提示。",
        "blocked": "方案存在参数冲突，不能应用。",
...
def _merge_actions(previous: list[dict[str, Any]], current: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for action in [*previous, *current]:
        if not _is_applicable_action(action):
            continue
        key = (str(action.get("type") or ""), str(action.get("target_field") or ""))
...
def _is_applicable_action(action: Any) -> bool:
    if not isinstance(action, dict) or action.get("type") not in APPLICABLE_ACTION_TYPES or not action.get("target_field"):
        return False
    if action.get("type") == "proposal_constraint":
