...
OPERATION_DOCS: dict[tuple[str, str], dict[str, str]] = {
    ...
    ("GET", "/api/reviews/{job_id}/generation-package"): _operation("生图准备", "获取 SolidWorks 生图参数包", "仅 ready 或 ready_with_warnings 状态返回冻结的 spring_generation_parameters/v2 参数快照。spring_parameters 包含线径、中径、自由长度、总圈数、有效圈数、旋向、两端磨削和端圈压并八个必填字段，以及可选的已确认材料字段 material；solidworks_preview.modelParameters 展示实际发送给 SolidWorks 的建模字段映射（不含尚未创建的 TaskId），其中有效圈数n为已确认有效圈数，是否磨平为整数 1 或 0。SolidWorks 根据中径和线径计算外径、内径。材料缺失或待确认时仍可生成，但参数包省略 material 并在就绪状态中给出警告。未执行标准化时 standard_context 保持空值，参数包仍可用于生图；其他未就绪状态返回 409 和具体原因。"),
    ...
    ("GET", "/api/reviews/{job_id}/generation-jobs"): _operation("生图任务", "查询审图单的全部生图版本", "返回指定审图单的全部生图任务，并标记最终版本及其参数修订是否与当前审图修订不同；历史完成版本仍可被选为最终版本。"),
    ...
    ("POST", "/api/generation-jobs/{generation_id}/preview/retry"): _operation("生图任务", "重新生成 PDF 对比预览", "对当前用户已完成、有 PDF 但没有 PNG 的任务重新渲染首页预览。预览失败不改变主任务 completed 状态；已存在 PNG 时幂等返回当前任务。"),
    ...
    ("POST", "/api/generation-jobs/{generation_id}/approve"): _operation("生图任务", "设为最终生图版本", "允许从该审图单任一已完成的历史生图任务中选定最终版本，不要求与当前审图修订一致；以所选任务创建时冻结的参数包和产物为准，不修改当前审图参数。同一审图单只保留一个最终版本，重复确认已选版本幂等返回。模拟任务会保留 is_mock 标识。"),
    ...
    ("GET", "/api/generation-jobs/{generation_id}/artifacts/{artifact_id}"): _operation("生成产物", "预览或下载生图任务产物", "按 artifact_id 获取指定任务的生成文件，并校验当前 ERP 用户的数据权限和安全路径。PDF 可使用 inline=1 在浏览器内预览；默认仍作为附件下载。"),
    ...
    ("POST", "/api/solidworks/status"): _operation("SolidWorks Worker", "接收 SolidWorks 状态和二维 PDF", "可信内网回调，无 Bearer 鉴权。TaskId 必须是我方分配的十位正数 Long。正常阶段使用 generating_3d、generating_2d 和 completed；三维或二维失败分别使用 generating_3d_error、generating_2d_error，只需提供非空 message，progress 可省略。成功接收（包括幂等重复和失败状态上报）固定返回 HTTP 200 与根级 {\"TaskId\":任务号,\"code\":200}。旧 failed 状态暂时保留兼容。completed 必须携带 mimeType=application/pdf、文件头正确且不含 Data URL 前缀的纯 Base64 二维 PDF；系统先尝试生成首页 PNG，再一次性登记产物并发布 completed。PNG 失败只影响 preview_status，不要求 SolidWorks 重传 PDF。相同 TaskId 和相同状态或PDF可幂等重试；不同终态或PDF冲突返回 409。若用户已取消任务，固定返回 HTTP 409 和根级 {\"TaskId\":任务号,\"code\":409}，SolidWorks应停止后续步骤并释放资源。未知 TaskId 返回 404，其他终态冲突仍返回标准 detail 错误结构。部署时必须仅向可信内网暴露该接口。"),
}
...
FIELD_DESCRIPTIONS = {
    ...
    "is_stale": "任务生成时的审图修订是否与当前修订不同；历史完成版本仍可选为最终版本。",
