...
def main() -> None:
    with tempfile.TemporaryDirectory() as directory:
        ...
        try:
            ...
            repository.create_review("delete001", review, artifact_dir=str(job_dir))

            result = api.delete_existing_review("delete001")
            assert result["deleted"] is True
            assert result["persistence"]["mode"] == "postgresql"
            assert result["artifact_cleanup"] == "deleted"
            assert repository.get_review("delete001") is None
