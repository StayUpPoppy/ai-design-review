...
def main() -> None:
    parser = argparse.ArgumentParser(description="Import surface treatment standard terms from Excel.")
    parser.add_argument("--xlsx", required=True, help="Path to 表面处理_术语对照标准表.xlsx")
    parser.add_argument("--output", default="config/surface_terms.json", help="Output JSON path")
    parser.add_argument("--sheet", default="术语对照标准", help="Worksheet containing the standard terms")
    args = parser.parse_args()

    xlsx_path = Path(args.xlsx)
    output_path = Path(args.output)
    terms = read_surface_terms_from_xlsx(xlsx_path, args.sheet)
    payload = {
    ...
    print(f"Imported {len(terms)} surface terms to {output_path}")


def read_surface_terms_from_xlsx(xlsx_path: Path, sheet_name: str) -> list[str]:
    ...
    try:
        ...
        raise ValueError(f"Worksheet {sheet_name} must contain headers: 类别, 名称") from exc

    terms: list[str] = []
    seen: set[str] = set()
    for row in rows[1:]:
        category = _cell(row, category_index)
        name = _cell(row, name_index)
        if category != "表面处理术语" or not name or name in seen:
            continue
        terms.append(name)
        seen.add(name)
    if not terms:
        raise ValueError(f"No 表面处理术语 rows found in worksheet: {sheet_name}")
    return terms
